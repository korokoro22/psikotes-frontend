...
export const storeAnswersKraepelin = (sessionId: number, data: string) =>
...
) => api.post(`/api/user/answers/mbti/${sessionId}`, data);

export const updateStatusTest = (sessionId: number) =>
  api.put(`/api/user/peserta/status/${sessionId}`);

export const triggerN8n = (pesertaId: number, tests: string) =>
  api.post(
    `/api/n8n/trigger/${pesertaId}`,
